...
inline void randomize(matrix& rhs) {
	std::for_each(rhs.begin(), rhs.end(), [](double& val) {
		val = rand() / (RAND_MAX + 1.);
	});
}
...
std::vector<double> neural_network::think(std::vector<double> input_arr) {
	...
	for (int i = 0; i < layer_sizes.size() - 1; i++) {
		...
		std::for_each(output.begin(), output.end(), neural_network::sigmoid);
		input = output;
	}

	std::for_each(output.begin(), output.end(), neural_network::desigmoid);
	return output;
}
...
void neural_network::train(std::vector<double>& training_data, std::vector<double>& training_target) {
	...
	std::for_each(deSigmoidOut.begin(), deSigmoidOut.end(), neural_network::desigmoid);
	...
	for (int i = layer_sizes.size() - 1; i > 0; i--) {
		...
		std::for_each(gradient.begin(), gradient.end(), [](double& val) -> double {
